...
class AsyncAPIBase:
    ...
    def __init__(self, token: Token, is_sandbox: bool):
        ...
        self.is_sandbox = is_sandbox
        self.token = token
        self.async_http = AsyncHttp(aiohttp.ClientSession(), self.timeout, token.get_string(), token.get_type())
...
class AsyncGuildRoleAPI(AsyncAPIBase):
    ...
    async def create_guild_role_member(
        ...
        response = self.async_http.put(url, request=JsonUtil.obj2json_serialize(role_req))
        return response.status_code == HttpStatus.ACTION_OK
    ...
    async def delete_guild_role_member(
        ...
        response = self.async_http.delete(url, request=JsonUtil.obj2json_serialize(role_req))
        return response.status_code == HttpStatus.ACTION_OK
...
class AsyncChannelPermissionsAPI(AsyncAPIBase):
    ...
    async def update_channel_permissions(
        ...
        if request.remove != "":
            request.remove = str(int(request.remove, 16))
        response = self.async_http.put(url, request=JsonUtil.obj2json_serialize(request))
        return response.status_code == HttpStatus.ACTION_OK
